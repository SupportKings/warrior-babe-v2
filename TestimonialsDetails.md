...
1) Entity Info
Entity name: client_testimonials
Display name: Client Testimonials
Main field: testimonial_type (or short content snippet)
Navigation path:
Standalone: dashboard/client-testimonials/:id
Or under client: dashboard/clients/:id/testimonials/:testimonialId

2) Page Header
Primary title: {client.name} - {testimonial_type}

3) Basic Information Sections
A. Testimonial Details - editable
Fields
client_id — lookup to Clients.name (required)
testimonial_type — text/select (required)
content — textarea (required)
testimonial_url — url (optional)
recorded_date — date (required)
recorded_by — lookup to Team Members.name (required, default current team member)

4) System Information (Auto-appended)
