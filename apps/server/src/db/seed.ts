...
const userFactory = (count: number) => {
	...
	return Array.from({ length: count }, (_, i) => {
		...
		return {
			...
			bio:
				Math.random() > 0.5
					? `Certified ${role.toLowerCase()} with extensive experience in fitness and wellness.`
					: null,
			calendar_link:
				Math.random() > 0.3
					? `https://calendly.com/${firstName.toLowerCase()}-${lastName.toLowerCase()}`
					: null,
		};
	});
};
...
const teamMemberFactory = (
	...
	return Array.from({ length: count }, () => {
		...
		const firstName =
			teamMemberFirstNames[
				Math.floor(Math.random() * teamMemberFirstNames.length)
			];
		const lastName =
			teamMemberLastNames[
				Math.floor(Math.random() * teamMemberLastNames.length)
			];
...
async function seed() {
	...
	try {
		...
		const teamMemberData = teamMemberFactory(
			TEAM_MEMBER_COUNT,
		...
		for (let i = 0; i < coachTeamData.length; i++) {
			...
			const premierCoachId =
				teamMemberIds[
					Math.floor(Math.random() * Math.min(teamMemberIds.length, 3))
				];
			const coachId =
				teamMemberIds[Math.floor(Math.random() * teamMemberIds.length)];
