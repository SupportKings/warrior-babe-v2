...
import type { SupabaseQueryConfig, TableRelationship, SortConfig } from '../types'
...
export function buildSupabaseQuery(
	...
	const { table, schema = 'public', select, filters, searchTerm, searchColumns, relationships, page, pageSize, sorting } = config
		...
		switch (type) {
			...
			case 'number':
				...
				if (operator === 'is') {
					...
					query = query.gte(columnId, numValue).lte(columnId, Number(values[1]))
				}
				break
