...
import { z } from 'zod'
import type { UniversalTableRow, UniversalTableConfig } from './types'

const filtersSchema = z.custom<FiltersState>()

interface UniversalDataTableWrapperProps<T extends UniversalTableRow> extends Omit<UniversalTableConfig<T>, 'enableUrlState' | 'urlStateKey'> {
	children: (state: {
...
export function UniversalDataTableWrapper<T extends UniversalTableRow>({
	...
	return <>{children({ 
		filters: filters || [], 
		setFilters: setFilters as React.Dispatch<React.SetStateAction<FiltersState>>
	})}</>
}