...
export function PaymentDisputeInfo({
	...
	const [formData, setFormData] = useState({
		...
		dispute_fee: dispute.dispute_fee || 0,
				...
				<div>
					...
					{isEditing ? (
						<Input
							...
							onChange={(e) => setFormData(prev => ({ ...prev, declined_at: e.target.value }))}
							className="mt-1"
						/>
					) : (
						<p className="text-sm">
				...
				<div>
					...
					{isEditing ? (
						<Select
							...
							onValueChange={(value) => setFormData(prev => ({ ...prev, disputed_status: value }))}
						>
						...
						<div className="text-sm">
							...
							<StatusBadge color={getDisputeStatusVariant(dispute.disputed_status)}>
				...
				<div>
					...
					{isEditing ? (
						<Input
							...
							onChange={(e) => setFormData(prev => ({ ...prev, dispute_fee: Number(e.target.value) }))}
							className="mt-1"
							step="0.01"
						/>
					) : (
				...
				<div>
					...
					{isEditing ? (
						<Input
							...
							onChange={(e) => setFormData(prev => ({ ...prev, stripe_charge_id: e.target.value }))}
							placeholder="Enter Stripe charge ID"
							className="mt-1"
							disabled
						/>
					) : (
						<p className="text-sm font-mono">{dispute.stripe_charge_id || "Not available"}</p>
					)}
				</div>
			</CardContent>
		</Card>
