...
import { usePayment } from "@/features/finance/queries/usePayments";
import { deletePaymentAction } from "@/features/finance/actions/deletePaymentDetail";
...
export default function PaymentDetailHeader({
	...
	const { execute: executeDeletePayment, isExecuting } = useAction(deletePaymentAction, {
		onSuccess: () => {
	...
	const handleDeletePayment = () => {
		executeDeletePayment({ id: paymentId });
	};

	// Extract display name from payment data
	const customerName = payment?.payment_slots?.[0]?.payment_plans?.clients?.name;
		...
		<div className="sticky top-0 z-10 flex h-[45px] flex-shrink-0 items-center justify-between border-border border-b px-4 py-2 lg:px-6">
			<div className="flex items-center gap-2">
				...
				<h1 className="font-medium text-[13px]">
					{displayName ? `${displayName} - Payment Details` : "Payment Details"}
				</h1>
			</div>
