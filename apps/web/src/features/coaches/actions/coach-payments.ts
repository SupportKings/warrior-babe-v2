  ...
  try {
    ...
    const formattedPeriods =
      finalActivityPeriods
        ...
        .map((period) => {
          const clientName =
            period.payment_plans?.clients?.name || "Unknown Client";
          const productName =
            period.payment_plans?.products?.name || "Unknown Product";
          // Use duration from payment plan or fall back to product default duration
          const duration =
            period.payment_plans?.products?.default_duration_months || 0;

          return {
            id: period.id,
            label: `${clientName} - ${productName} - ${duration} Months`,
