...
import { z } from "zod";
import { type UpdateCoachInput, updateCoach } from "../actions/update-coach";
...
export function CoachEditForm({ coach, onSuccess }: CoachEditFormProps) {
	...
	const initialRoles = coach.user?.role
		? coach.user.role.split(",").map((r) => r.trim())
		: [];
	...
	const form = useForm({
		...
		onSubmit: async ({ value }) => {
			...
			const submitData: UpdateCoachInput = {
				id: coach.id,
				...value,
